...
class Trainer:
    ...
    def createDataLoader(self, source_path):
        ...
        X_train = torch.from_numpy(np.load(source_path + "X_train_VBM_OpenBHB.npy")).permute(0,4,1,2,3)
        Y_train = np.load(source_path + "Y_train_VBM_OpenBHB.npy")

        X_test = torch.from_numpy(np.load(source_path + "X_test_VBM_OpenBHB.npy")).permute(0,4,1,2,3)
    ...
    def trainEpoch(self):
        ...
        self.saveCheckpoint(self.sfcnModel,self.optimizer,model_path,
